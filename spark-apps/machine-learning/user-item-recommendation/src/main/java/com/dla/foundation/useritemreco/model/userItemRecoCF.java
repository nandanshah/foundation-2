...
public enum userItemRecoCF {
	ID("id"), PROFILE("profileid"), TENANT("tenantid"), REGION("regionid"), ITEM(
			...
			"fpscore"), FP_SCORE_REASON("fpscorereason"), NORMALIZED_NEW_SCORE(
			"normalizednewscore"), NEW_RELEASE_SCORE("newscore"), NEW_RELEASE_SCORE_REASON(
			"newscorereason"), SOCIAL_SCORE("socialscore"), SOCIAL_SCORE_REASON(
			"socialscorereason"), PIO_SCORE("recoByfoundation"), PIO_SCORE_REASON(
			"recoByfoundationscorereason"), DATE("date"), EVENT_REQUIRED(
			"eventrequired"), PREFERRED_REGION("preferredregion"), ACCOUNT(
			"accountid"), PIO_DATE("lastrecofetched");
